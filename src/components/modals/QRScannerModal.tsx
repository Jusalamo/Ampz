...
export function QRScannerModal({ isOpen, onClose, userId, onCheckInSuccess }: QRScannerModalProps) {
  ...
  const processQRCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      const result = await processQRCodeScan(code.trim(), 'public');
      ...
      if (result.success) {
        ...
        if (result.error?.includes('inside the event\'s geofence')) {
          ...
          setErrorMessage(result.error || 'Invalid QR code or access code');
          setStep('error');
        }
      }
    } catch (err: any) {
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          ...
          {step === 'code' && (
            <>
              ...
              <p className="text-muted-foreground text-center mb-6">
                Enter the QR code content or event access code
              </p>
              <Input
                value={manualCode}
                onChange={(e) => setManualCode(e.target.value)}
                placeholder="Enter QR code content or access code..."
                className="text-center text-lg font-mono h-14 max-w-xs mb-6"
                autoFocus
              />
          ...
          {step === 'verifying' && (
            <>
              ...
              <h3 className="text-xl font-bold mb-2">Verifying QR Code</h3>
              <p className="text-muted-foreground text-center">Checking code and location...</p>
            </>
          )}
          ...
          {step === 'success' && (
            <>
              ...
              <Button className="h-12 px-8" onClick={handleClose}>Done</Button>
            </>
          )}
          ...
          {step === 'error' && (
            <>
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="h-12" onClick={handleTryAgain}>
                  Try Again
                </Button>
                <Button className="h-12" onClick={() => setStep('code')}>
                  Enter Code
                </Button>
              </div>
            </>
