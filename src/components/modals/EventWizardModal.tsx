...
export function EventWizardModal({ isOpen, onClose }: EventWizardModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col overflow-hidden">
      ...
      <div 
        ref={contentRef}
        className="flex-1 overflow-y-auto p-5 pb-32" // Added pb-32 for CTA space
      >
        {/* Step 1: Basic Info */}
        {step === 1 && (
        ...
        {step === 3 && (
          <div className="space-y-4 animate-fade-in h-full">
            ...
            <p className="text-xs text-muted-foreground text-center">
              Tap on the map or drag the marker to set location • Coordinates: {eventData.coordinates.lat.toFixed(4)}, {eventData.coordinates.lng.toFixed(4)}
            </p>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="glass-card p-4 rounded-lg">
              <p className="text-center text-3xl font-bold mb-1" style={{ color: eventData.themeColor }}>
                {eventData.geofenceRadius}m
              </p>
              <p className="text-center text-sm text-muted-foreground">Check-in radius</p>
            </div>

            <div className="space-y-4">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">Radius Distance</span>
              ...
              <div className="relative w-full h-[300px] rounded-xl overflow-hidden border-2 border-border mt-4">
                {isMapLoading && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-900 z-10">
                    <Loader2 className="w-8 h-8 animate-spin text-primary" />
                    <span className="ml-2 text-sm">Loading map...</span>
      ...
      {step < 6 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 border-t border-border bg-background/95 backdrop-blur-sm z-10">
      ...
      {step === 6 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 border-t border-border bg-background/95 backdrop-blur-sm z-10">
          <Button 
            className="w-full rounded-lg h-12 max-w-2xl mx-auto block" 
            onClick={onClose}
            style={{ 
              backgroundColor: eventData.themeColor,
              borderColor: eventData.themeColor 
